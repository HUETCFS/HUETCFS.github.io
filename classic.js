...
function FormFacade(data)
{
        ...
        sitems.forEach(function(item, i){
            ...
            if(oitem)
            {
                if(oitem.calculated)
                {
                    ...
                    if(disp) disp.value = item.format?item.format(calcval):calcval;
                }
    ...
    this.renderUploadField = function(id, entry, files)
    {
        ...
        .use(Uppy.AwsS3, {
            ...
            getUploadParameters(file) {
                ...
                if(!savedId)
                    prm = curr.saveDraft().then(function(jso){ return jso.savedId; });
    ...
    this.render = function()
    {
        ...
        if(!styelm)
        {
            ...
            document.getElementsByTagName('body')[0].appendChild(styelm);
            styelm.innerHTML = ejs.render(this.template.style, this);
        }
        var elm = this.getContentElement();
    	if(elm)
        {
            elm.innerHTML = ejs.render(this.template.text, this);
            this.renderUpload();
        }
    ...
    this.submit = function(frm, secid)
    {
        ...
        if(this.draft.draftSeq)
            pairs.draftSeq = this.draft.draftSeq;
        pairs.pageHistory = this.getPageHistory();
        if(this.config && this.config.plan=='blocked')
            pairs.plan = 'blocked';
        ...
        this.sendData(pairs).then(function(rs){
            ...
            if(rs && rs.code==200)
            {
                ...
                if(curr.data.facade && curr.data.facade.whatsapp)
                {
                    var ph = '16506996650';
                    if(curr.data.facade.whatsapp.phone)
                        ph = curr.data.facade.whatsapp.phone.match(/\d+/g).join('');
                    curr.render();
                    if(!smtxt)
    ...
    this.getSections = function()
    {
        ...
        if(frm && frm.items)
        {
            var secs = this.asSections(frm.items);
            secs[0].title = frm.title;
            secs[0].description = frm.description;
            return secs;
        }
        else if(frm)
        {
            var secs = [{id:'root', items:[]}];
            secs[0].title = frm.title;
            secs[0].description = frm.description;
            return secs;
        }
            ...
            payform.addEventListener('submit', function(ev) {
              ...
              stripe.confirmCardPayment(data.clientSecret, {
                ...
                else
                {
                  ...
                  {
                    curr.draft.paymentId = result.paymentIntent.id;
                    var frmid = 'Publish'+curr.data.request.params.publishId;
                    var rfrm = doc.getElementById(frmid);
...
FormFacade.prototype.calculateEngine = function (items, entr, tmpl, citm)
{
    ...
    params.price = params.PRICE = function(val, currency='$')
    {
        if(val)
        {
            ...
            return val.map(function(txt){
                //if(isNaN(txt)==false) return Number(txt);
        ...
        secs.forEach(function(sec, s){
            sec.items.forEach(function(item, i){
                ...
                if(item.type=='TEXT')
                {
                    var qntstr = entr[item.entry];
                    var qnt = qntstr&&isNaN(qntstr)==false?Number(qntstr):0;
                    if(amt && qnt) lines.push([item.title, amt, qnt]);
                }
                else if(item.type=='LIST' || item.type=='MULTIPLE_CHOICE' || item.type=='SCALE')
                {
                    if(amt>0)
                    ...
                    else
                    {
                        var amtstr = entr[item.entry];
                        amt = params.price(amtstr, currency);
                        if(amt>0) lines.push([item.title, amt, 1]);
                    }
                }
                else if(item.type=='CHECKBOX')
                {
                    if(amt>0)
                    {
                        ...
                        qntstrs.forEach(function(qntstr){
                            var qnt = qntstr?(isNaN(qntstr)==false?Number(qntstr):1):0;
                            if(amt && qnt) lines.push([qnt==1?(item.title+' - '+qntstr):item.title, amt, qnt]);
                        });
                    }
                    else
                    {
                        ...
                        vals.forEach(function(val){
                            var vlamt = params.price(val, currency);
                            if(vlamt>0) lines.push([item.title, vlamt, 1]);
                        });
                    }
                }
                else if(item.type=='GRID')
                    ...
                    rws.forEach(function(rw, rwi){
                        ...
                        item.choices.forEach(function(ch, chi){
                            if(ch && ch.value && valmap[ch.value])
                            {
                                ...
                                {
                                    var qnt = ch.value?(isNaN(ch.value)==false?Number(ch.value):1):0;
                                    if(amt && qnt) lines.push([ttl, amt, qnt]);
                                }
                                else
                                {
                                    var rwamt = params.price(rw.value, currency);
                                    ...
                                    {
                                        var qnt = ch.value?(isNaN(ch.value)==false?Number(ch.value):1):0;
                                        if(rwamt && qnt) lines.push([ttl, rwamt, qnt]);
                                    }
                                } 
                            }
                        });
    ...
    params.score = params.SCORE = function()
    {
        ...
        if(!scope) scope = true;
        var itms;
        if(scope==true || scope==params.ALL || scope==params.SECTION || scope.getMetadata)
